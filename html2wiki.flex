...
\&lt;                         cout<<"<";
\&amp;                        cout<<"&";
\<TITLE\>[^<]*\<\/TITLE\>     {string title=yytext; int pos1 = title.find_first_of(">"); int pos2 = title.find_first_of("<",pos1+1); title=title.substr(pos1+1, pos2-pos1-1); if(title!="") cout<<"%title "<<title<<endl;}
...
<ALINK>[^<]*\<\/A\>            link_text=yytext; link_text=link_text.substr(0,link_text.size()-4); if(link_href.find("http")==string::npos) cout<<"\[\["<<link_href.substr(0,link_href.size()-5)<<"\]\["<<link_text<<"\]\]";else cout<<"\["<<link_href<<" "<<link_text<<"\]"; BEGIN(INITIAL);
<ALINK>(.|\n)         
\n\n(\n)*                       cout<<endl<<endl;
